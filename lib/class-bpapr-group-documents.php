...
class BPAPR_Documents {
	...
	public function add_cssjs_hooks_handler() {
		...
		wp_enqueue_script( 'bpfb_group_documents', BPFB_PLUGIN_URL . '/assets/js/bpfb_group_documents.js', array( 'ps-activity-plus' ) );
		wp_localize_script( 'bpfb_group_documents', 'l10nBpfbDocs', array(
			'add_documents'     => __( 'Dokumente hinzufügen', 'ps-activity-plus' ),
			'no_group_selected' => __( 'Bitte wähle eine Gruppe aus, in die Du hochladen möchtest', 'ps-activity-plus' ),
		) );
	}
	...
	public function create_documents_tag( $docs ) {
		...
		return sprintf(
			__( '%s Datei(en) hochgeladen: %s in %s', 'ps-activity-plus' ),
