...
class BPAPR_Preview_Handler {
	...
	public function ajax_preview_video() {
		...
		$warning  = __( 'Bei der Verarbeitung Deiner Anfrage ist ein Fehler aufgetreten', 'ps-activity-plus' );
		$response = $url ? __( 'Verarbeitung...', 'ps-activity-plus' ) : $warning;
	...
	private function get_remote_contents( $url ) {
		$response = wp_remote_get(
			$url,
			array(
				'user-agent' => 'PS BuddyPress Activity', // Some sites will block default WP UA.
			)
		);
